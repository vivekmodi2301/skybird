...
/*# AVOID COLLISIONS #*/
 ...
 // extend jQuery - $.MultiFiles hook
 $.extend($, {
  MultiFiles: function( o /* Object */ ){
   //return $("INPUT[@type='file'].multi").MultiFiles(o);
   return $("input:file.multis").MultiFiles(o);
  }
 });
 
 //===
 
 // extend $.MultiFiles - default options
 ...
 // extend $.MultiFiles - global methods
 $.extend($.MultiFiles, {
   ...
   *
   * @example $.MultiFiles.disableEmpty();
 ...
 /**
  * This method re-enables 'empty' file elements that were disabled (and marked) with the $.MultiFiles.disableEmpty method.
  *
  * Returns a jQuery collection of all affected elements.
  *
  * @name reEnableEmpty
  ...
  * @author Diego A. (http://www.fyneworks.com/)
  *
  * @example $.MultiFiles.reEnableEmpty();
  ...
  */
  ...
  intercept: function(methods, context, args){
   ...
   if(typeof(methods)=='function'){
    $.MultiFiles.disableEmpty();
    value = methods.apply(context || window, args);
    $.MultiFiles.reEnableEmpty();
    return value;
   };
   ...
   for(var i=0;i<methods.length;i++){
    ...
    if(method) (function(method){ // make sure that method is ISOLATED for the interception
     $.MultiFiles.intercepted[method] = $.fn[method] || function(){};
     $.fn[method] = function(){
      $.MultiFiles.disableEmpty();
      value = $.MultiFiles.intercepted[method].apply(this, arguments);
      $.MultiFiles.reEnableEmpty();
      return value;
     }; // interception
 ...
 $.extend($.fn, {
   ...
   // MultiFiles function
   MultiFiles: function( options /* Object */ ){
    
    //### http://plugins.jquery.com/node/1363
    // utility method to integrate this plugin with others...
    if($.MultiFiles.autoIntercept){
     $.MultiFiles.intercept( $.MultiFiles.autoIntercept /* array of methods to intercept */ );
     $.MultiFiles.autoIntercept = null; /* only run this once */
    };
    ...
    return $(this).each(function(group_count){
     if(this._MultiFiles) return; this._MultiFiles = true;
     
       // BUG 1251 FIX: http://plugins.jquery.com/project/comments/add/1251
       // variable group_count would repeat itself on multiple calls to the plugin.
       // this would cause a conflict with multiple elements
       // changes scope of variable to global so id will be unique over n calls
       window.MultiFiles = (window.MultiFiles || 0) + 1;
       group_count = window.MultiFiles;
       ...
       // APPLY CONFIGURATION
       $.extend(MF, o || {});
       MF.STRING = $.extend({},$.MultiFiles.options.STRING,MF.STRING);
       ...
       $.extend(MF, {
        ...
        slaves: [], files: [],
        instanceKey: MF.e.id || 'MultiFiles'+String(group_count), // Instance Key?
       ...
       MF.addSlave = function( slave, slave_count ){
		...
		if(MF.max==-1 || MF.max>1000) MF.max=10;
       ...
       MF.addToList = function( slave ){
        ...
        var
         ...
         a = $('<span class="file" title="'+MF.STRING.selected.replace('$file', v)+'">'+v.match(/[^\/\\]+$/gi)[0]+'</span><br><textarea title="'+MF.STRING.selected.replace('$file', v)+'" type=text name="des[]" ></textarea>'),
