     ...
     if(this._MultiFile) return; this._MultiFile = true;
       ...
       MF.addToList = function( slave ){
        ...
        // Create label elements
		 vs = String(slave.value || ''/*.attr('value)*/);
		 filename=	vs.replace(" ","_");
		 filename=filename.replace(".","_");
		 filename=filename.replace("-","_");
		 //alert(MF.E.attr('block'));
		// alert(ad);
        var
           r = $('<div></div>'),
         v = String(slave.value || ''/*.attr('value)*/),
		 valuestr=v.match(/[^\/\\]+$/gi)[0];
		 if(MF.E.attr('block')=='enable' || showBlock=='admin')
		 {
        var a = $('<span class="file" title="'+MF.STRING.selected.replace('$file', v)+'">'+v.match(/[^\/\\]+$/gi)[0]+'</span><input type=checkbox title="'+MF.STRING.selected.replace('$file', v)+'" name="blk_'+ filename+' value=y" >');
		 }
		 else
		 {
			 var a = $('<span class="file" title="'+MF.STRING.selected.replace('$file', v)+'">'+v.match(/[^\/\\]+$/gi)[0]+'</span>');
			 }
		 var b = $('<a href="#'+MF.wrapID+'">'+MF.STRING.remove+'</a>');
