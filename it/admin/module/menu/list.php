...
<table border="1px" style="border:1px #000 solid; border-collapse:collapse; margin-top:50px;" align="center" width="600px">
	<tr>
    	<td colspan="4" style="text-align:center; font-size:20px;" height="30px"> Our Services List</td>
    </tr>
    <tr>
  <form id="frm" method="post">
    ...
    <tr>
		<td align="right" colspan="4">
        	<a href="index.php?mod=menu&do=add_edit">Add  Catagory</a>
        </td>
	</tr>
    <tr>
    	<td>Id</td>
        
 <th onClick="changeOrder('name')"> Catagory 
        <?php 
	...
	$frmdataget=$_REQUEST;
	PaginationWork();
	$product_count=mysql_query('SELECT COUNT(1) AS tot FROM menu LIMIT 1') ;
	$totRslt=mysql_fetch_array($product_count);
	$product_record=mysql_query("SELECT  * FROM menu  $wh order by $order $by LIMIT ".$frmdata['from'].", ".$frmdata['to']);
	while($product_data=mysql_fetch_array($product_record))
	{$i++; ?>
    <tr>
        <td><?php echo $product_data['id'];?></td>
        <td><?php echo $product_data['name'] ?></td>
        <td><a href="index.php?mod=menu&do=add_edit&id=<?php echo $product_data['id']; ?>" >Edit</a></td>
...
function del(id)
{
	if(confirm("Do you want to delete record"))
		location.href="index.php?mod=menu&do=list&id="+id;
}
</script>